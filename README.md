...
## 勤怠一覧の詳細表示に関して
* デザイン画では打刻無い日でもユーザーと管理者の両方に詳細が計上されているが、勤怠アプリとして良い設計とは言えない為下記のように設定した。
   * ユーザー：勤怠一覧に表示は見えるがさわれないように画面ロック。
   * 管理者：一覧の詳細全て表示、過去における打刻忘れは全て管理者で修正、未来は記入できないよう記入ロック。

## テストケース一覧に関して
* 勤怠詳細情報修正機能（一般ユーザー）のエラーメッセージに関して、出勤時間が退勤時間より後になっている場合、エラーメッセージが機能要件内のエラーメッセージと異なるため、機能要件エラーメッセージを採用している。
