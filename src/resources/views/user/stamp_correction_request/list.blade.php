        ...
        <nav class="request-tab">
            <ul class="request-tab__list">
                <li class="request-tab__item">
                    <a href="{{ route('stamp_correction_request.list', ['status' => $pendingStatus]) }}"
                        class="{{ $status === $pendingStatus ? 'is-active' : '' }}">
                        承認待ち
                    </a>
                </li>
                <li class="request-tab__item">
                    <a href="{{ route('stamp_correction_request.list', ['status' => $approvedStatus]) }}"
                        class="{{ $status === $approvedStatus ? 'is-active' : '' }}">
                        承認済み
                    </a>
                </li>
            </ul>
            ...
            <tbody class="request__body-group">
                @foreach ($requests as $req)
                    <tr class="request__data-row">
                        ...
                        <td class="request__data request__data--remarks">
                            {{ $req->detailAfter('remarks') }}
                        </td>
                        <td class="request__data">{{ $req->created_at->format('Y/m/d') }}</td>
                        <td class="request__data">
                            <a class="request__data-link" href="{{ route('attendance.detail', $req->attendance->id) }}">
                                詳細
                            </a>
                        </td>
                    </tr>
